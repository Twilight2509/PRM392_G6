...
import com.example.mychatapp.network.ApiClient;
...
public class ChatActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Thêm image picker launcher để chọn ảnh
                ...
                result -> {
                    if (result.getResultCode() == RESULT_OK && result.getData() != null) {
                        ...
                        try {
                            ...
                            sendImageMessage(bitmap);  // Gửi tin nhắn hình ảnh
                        } catch (IOException e) {
                            e.printStackTrace();
    ...
    private void sendImageMessage(Bitmap bitmap) {
        // Nén ảnh và chuyển đổi thành chuỗi Base64
        ...
        message.put(Constants.KEY_IMAGE, encodedImage); // Sử dụng key khác cho ảnh
        ...
        if (value != null) {
            ...
            for (DocumentChange documentChange : value.getDocumentChanges()) {
                if (documentChange.getType() == DocumentChange.Type.ADDED) {
                    ...
                    chatMessage.image = documentChange.getDocument().getString(Constants.KEY_IMAGE); // Lấy chuỗi ảnh nếu có
    ...
    private void setListeners() {
        ...
        binding.btnImage.setOnClickListener(v -> openImagePicker()); // Thêm nút mở bộ chọn ảnh
    }
